...
| 2026-05-25 | Landing-page scroll story codified two reusable patterns: **StaticFlow vs CinematicFlow path-selection** and the **tri-state Reveal** model. Components live in `components/landing/`; hooks in `hooks/useReducedMotion.ts`, `hooks/useInView.ts`, `hooks/useScrollProgress.ts`. | **Path-selection pattern:** every motion-heavy section forks at the top level on `useReducedMotion()` and/or viewport width. Reduced-motion + small-viewport users get a `StaticFlow` (stacked, no scrub, no pin); everyone else gets the `CinematicFlow` (sticky-pinned, scrub-driven). Same copy, same component, two render paths — never gate animation inside a single render tree with conditional CSS, because the math still runs and reduced-motion users still pay the cost. Used in `verify-flow-section.tsx`. **Tri-state Reveal:** `<Reveal>` wraps any element and renders one of three states — `pristine` (SSR + first paint, no opacity transition), `hidden` (post-hydration, before IntersectionObserver fires), `revealed` (one-shot, never reverses). Pristine state matters: a two-state `hidden → revealed` model causes above-the-fold content to flash invisible on hydration when JS arrives faster than the observer. The pristine state defers the `opacity: 0` until after first paint, so SSR'd above-the-fold content never blanks. **Scrub math is pure:** `clamp01`, `progressToStep`, `subProgress` live in `landing-section-data.ts` and are Jest-tested independent of the DOM. This is the precedent for any future scrub-driven section — the math must be unit-testable without rendering React. |
